...
# Season stacker by Phil Marshall (dr.phil.marshall@gmail.com), modified
# by Humna Awan (humna.awan@rutgers.edu)
    ...
    def _run(self, simData):
        ...
        for i, ack in enumerate(simData):
            ...
            ra_geo[i], dec_geo[i] = palpy.mapqk(simData[self.raCol][i], simData[self.decCol][i],
                                                0., 0., 0., 0., mtoa_params)
        x_geo1, y_geo1 = self._gnomonic_project_toxy(
            ra_geo1, dec_geo1, simData[self.raCol], simData[self.decCol])
...
class SeasonStacker(BaseStacker):
    ...
    def _run(self, simData):
        ...
        # 0.5 to go from RA to month; 365.25/12.0 for months to days
        daysSinceEquinox = 0.5*objRA*(365.25/12.0)
